...
import {
  ...
  Check, X, ShoppingCart, Zap, BarChart3
} from 'lucide-react';
...
export const CampaignConfigPage = () => {
  ...
  const handleSubmitForReview = () => {
    handleSave();
    if (errors.length === 0 && activeCampaign) {
                ...
                <div className="grid grid-cols-7 gap-1 text-center">
                  ...
                  {calendarDays.map((day, index) => {
                    ...
                    const hasCampaign = campaigns.some(c => 
                      isDateInRange(day, c.startTime, c.endTime)
                    );
                    ...
                    return (
                      <div
                        key={index}
                        className={cn(
                          'h-9 flex items-center justify-center text-sm rounded-lg transition-colors cursor-pointer',
                          isToday && 'bg-[#1e3a5f] text-white font-bold',
                          hasCampaign && !isToday && 'bg-amber-100 text-amber-700',
                          !hasCampaign && !isToday && 'text-slate-600 hover:bg-slate-100'
                        )}
                      >
                        {day.getDate()}
                      </div>
